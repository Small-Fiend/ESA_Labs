<template>
  <div>
    <v-row>
      <v-col>
        <h1> DanceStudios table</h1>
      </v-col>
    </v-row>
    <v-row>
      <v-col>
        <v-btn @click="addNewRowStart">Add new danceStudio</v-btn>
      </v-col>
    </v-row>
    <v-simple-table>
...
import { springGetAll, springDeleteItem, springAddNewItem, springUpdateItem } from "../endpoints/table_danceStudios_endpoints";
import DialogExample from "./DialogExample.vue";
export default {
  components: { DialogExample },
  name: "DanceStudiosTable",
  data() {
    return {
  ...
  methods: {
    ...
    editRowStart(item) {
      this.title = `Modify existing danceStudio #${item.id}`
    ...
    addNewRowStart() {
      this.title = 'Add new danceStudio'
      // Defines fields we nspringd to fill in the dialog
