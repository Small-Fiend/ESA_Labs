...
<template xmlns:v-bind="http://www.w3.org/1999/xhtml">
  <div>
    ...
    <v-row>
      <v-col>
        <v-btn @click="addNewRowStart">Add new dancer</v-btn>
      </v-col>
    </v-row>
    <v-simple-table>
      <template v-slot:default>
        ...
        <tbody>
          <tr v-for="item in table_data" v-bind:key="item.id">
            <td>{{ item.id }}</td>
            <td>{{ item.Name }}</td>
            <td>{{item.dancerClass.id}}</td>
            <td>{{item.dancerStudio.id}}</td>
            <td>
...
import { eeGetAll, eeDeleteItem, eeAddNewItem, eeUpdateItem } from "../endpoints/table_dancers_endpoints";
...
export default {
  ...
  data() {
    return {
      table_header: ["Id", "Name", 'DancerClass ID', 'DancerStudio ID'],
      table_data: [],
      showDialog: false,
      newItem: {address: ''},
      editingItemIndex: -1,
  ...
  methods: {
    ...
    editRowStart(item) {
      ...
      this.newItem = {'name': item.name, 'classId': item.danceClass.id, 'studioId': item.danceStudio.id}
